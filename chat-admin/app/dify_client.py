...
import httpx
from app.services.system_config import get_dify_api_key, get_dify_base_url

logger = logging.getLogger("chat_gateway")


def _base_url(system_id: str | None = None) -> str:
    url = get_dify_base_url(system_id)
    return f"{url}/v1"


def _headers(system_id: str | None = None) -> dict:
    return {
        "Authorization": f"Bearer {get_dify_api_key(system_id)}",
