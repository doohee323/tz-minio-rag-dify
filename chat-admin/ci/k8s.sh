...
setup_environment() {
    ...
    # Set database host by branch (used only for drillquiz-style apps)
    if [ "${clean_branch}" = "main" ]; then
...
# Frontend build function (tz-cointutor style)
build_frontend() {
    ...
    image_frontend="doohee323/chat-admin-frontend:${BUILD_NUMBER}"
    DOCKER_BUILDKIT=1 docker build -f Dockerfile.frontend --progress=plain -t ${image_frontend} .
    ...
    # Copy to static/ (FastAPI serves from static/)
    rm -rf static
    mkdir -p static
    cp -Rf frontend-dist/* static/
    cp -f static/index.html static/404.html 2>/dev/null || true
    
    rm -rf frontend-dist
    log_info "✅ Frontend build completed (static/ populated, use Dockerfile.backend for image)"
}
...
deploy_to_kubernetes() {
    ...
    # Create kubeconfig Secret from Jenkins KUBECONFIG (trigger-reindex API uses kubectl)
    if [ -n "${KUBECONFIG:-}" ] && [ -f "${KUBECONFIG}" ]; then
        log_info "Creating chat-admin-kubeconfig Secret from KUBECONFIG (Jenkins env)"
        kubectl -n ${NAMESPACE} create secret generic chat-admin-kubeconfig-${SECRET_SUFFIX} \
            --from-file=config="${KUBECONFIG}" \
            --dry-run=client -o yaml | kubectl -n ${NAMESPACE} apply -f -
    else
        log_info "KUBECONFIG not set or file not found - trigger-reindex will use in-cluster config (RBAC) or fail"
    fi
