#!/usr/bin/env bash
# .env에 넣을 CHAT_GATEWAY_JWT_SECRET, CHAT_GATEWAY_API_KEYS 예시 값을 생성해서 출력합니다.
# 사용: ./scripts/gen-env-values.sh  → 출력을 보고 .env에 복사

echo "# 아래 값을 .env에 넣으세요."
echo ""
echo "CHAT_GATEWAY_JWT_SECRET=$(openssl rand -hex 32)"
echo "CHAT_GATEWAY_API_KEYS=key_drillquiz_$(openssl rand -hex 8),key_cointutor_$(openssl rand -hex 8)"
echo ""
