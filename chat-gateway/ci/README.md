...
3. **Build & Push Image** – Builds Docker image from `./chat-gateway` and pushes to `REGISTRY/chat-gateway:BUILD_NUMBER` (e.g. Docker Hub).
4. **Deploy to Kubernetes** – Runs `chat-gateway/ci/k8s.sh` with `BUILD_NUMBER`, `GIT_BRANCH`, `NAMESPACE`, and action `deploy`.
...
Run from repo root or from `chat-gateway/`:
...
The script uses `k8s.yaml` and, when applicable, `k8s-dev.yaml` or `k8s-qa.yaml` to deploy the chat-gateway app (and related resources) into the cluster. When ArgoCD is enabled, it can update the ArgoCD repo and sync the application.
...
| Credential ID | Usage |
|---------------|--------|
| `kubeconfig-jenkins` | File credential: kubeconfig for cluster access |
...
- **Deploy fails in Jenkins**  
  Ensure `kubeconfig-jenkins` exists and the cluster is reachable. Check that `chat-gateway/ci/k8s.sh` is present and executable in the workspace.
